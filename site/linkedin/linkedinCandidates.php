...
<div class="row">
    ...
    <br>
        <form action="csv_v2/importDatalinkedin.php" method="post" enctype="multipart/form-data">
            <input type="file" name="filelinkedin" />
            <br>
            <input type="submit" class="btn btn-primary" name="importSubmitlinkedin" value="IMPORT">
        </form>
        <br>
    </div>
    <div class="col-md-12 head">
            ...
            <table class="table">
                <thead>
                    ...
                    <th style="color:black;display:flex;"> <input type="checkbox" id="selectall">&nbsp;Select</th>
                        ...
                        <th><input type="text" class="form-control" placeholder="Current company" disabled></th>
                        <!-- <th><input type="text" class="form-control" placeholder="Preferred_loc" disabled></th>
                        <th><input type="text" class="form-control" placeholder="Industry" disabled></th>
                        <th><input type="text" class="form-control" placeholder="Work_Segment" disabled></th> -->
...
// echo $skills;
    ...
    if($bs=='' && !($a==0 &&$b==0 &&$c==0 &&$d==0)){
        $sql = "SELECT * FROM Linkedin WHERE (curr_ctc BETWEEN $c and $d) and (experience BETWEEN $a and $b);";

    }
    else if($a==0 &&$b==0 &&$c==0 &&$d==0&& $bs!=''){
        $sql = "SELECT *, MATCH (tech,cv_parsed) AGAINST ('$bs' IN NATURAL LANGUAGE MODE) AS score FROM Linkedin WHERE MATCH (tech,cv_parsed) AGAINST ('$bs' IN NATURAL LANGUAGE MODE);";
    }
    else if($a==0 &&$b==0 &&$c==0 &&$d==0 &&$bs==''){
    ...
    {
        $sql = "SELECT *, MATCH (tech,cv_parsed) AGAINST ('$bs' IN NATURAL LANGUAGE MODE) AS score FROM Linkedin WHERE MATCH (tech,cv_parsed) AGAINST ('$bs' IN NATURAL LANGUAGE MODE) and (curr_ctc BETWEEN $c and $d) and (experience BETWEEN $a and $b);";
...
<script>
    ...
    $('#exportbtn').html('Saved!');
        ...
        window.location.href = "http://<?php  echo $_SERVER['SERVER_NAME']; ?>/jobs/site/exportshortliststudlinkedin.php";
        }
...
function writeStudents(){
    ...
    if ($result ->num_rows >0) {
        ...
        while($row1 = $result->fetch_assoc()) {
            ...
            <td  ><a href="mailto:<?php echo $row1["email"];?>"><?php echo $row1["email"];?></a></td>
            <!-- <td  ><?php echo $row1["curr_comp"];?></td> -->
            <td  ><a href="../specialty/uploads/linkedin/<?php echo $row1['student_id']; ?>/<?php echo $row1["resume"];?>" target="blank">View</a></td>
            <td  ><button id="<?php echo $row1['student_id'];?>" onclick="shortlist(this.id);" style="background:transparent;color:black">Shortlist</button></td>
