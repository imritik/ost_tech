...
if($query->num_rows ==1){
    ...
    while($row = $query->fetch_assoc()){
      $resumelinks='http://talentchords.com/jobs/specialty/uploads/'.$studlist[$x].'/'.$row['resume'];
        $lineData = array($row['student_id'], $row['stud_name'], $row['email'],$row['contact'], $row['pass'],$row['Alternate_emails'],$row['total_exp'],$row['curr_company'], $row['ctc_fixed'],$row['ctc_variable'],$row['curr_ctc'],$row['expected_ctc'], $row['desgination'],$row['notice_period'],$row['curr_loc'],$row['preferred_loc'],$row['prev_comp'],$row['prev_comp_other'],$row['ug_college'],$row['ug_degree'],$row['pg_college'],$row['pg_degree'],$row['add_courses'],$resumelinks,$row['cv_parsed'] ,$row['ug_city'],$row['ug_agg'],$row['ug_yoc'],$row['pg_city'],$row['pg_agg'],$row['pg_yoc'],$row['linkedin'],$row['fb'],$row['twitter'],$row['is_active'],$row['turnup_rate'],$row['Offer_acceptance_rate'],$row['source'],$row['callers_comment'],$row['Uploaded_by'],$row['tech'],$row['updated_on'], $row['modified_on'],$row['	cv_upload_date'],$row['latest_application_date']);
